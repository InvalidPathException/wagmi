...
use wagmi::{
    Error, ExportValue, Imports, Instance, Module, RuntimeFunction, RuntimeSignature, Signature,
    ValType, WasmGlobal, WasmMemory, WasmTable, WasmValue,
};
    ...
    for (name, ex) in &inst.module.exports {
        if matches!(ex.extern_type, wagmi::module::ExternType::Func) {
            ...
            match src {
                ...
                RuntimeFunction::OwnedWasm { runtime_sig: ty, .. }
                | RuntimeFunction::ImportedWasm { runtime_sig: ty, .. } => {
                    // For wasm-backed exports, expose an owner handle that
                    // delegates execution into the owning instance
                    out.insert(
                        name.clone(),
                        ExportValue::Function(RuntimeFunction::ImportedWasm {
                            runtime_sig: ty,
                            owner: weak.clone(),
                            function_index: fi,
                        }),
...
fn spectest_exports() -> HashMap<String, ExportValue> {
    ...
    exports.insert(
        "global_i32".into(),
        ExportValue::Global(Rc::new(WasmGlobal {
            ty: ValType::I32,
            mutable: false,
            value: Cell::new(WasmValue::from_u32(666)),
        })),
    );
    exports.insert(
        "global_i64".into(),
        ExportValue::Global(Rc::new(WasmGlobal {
            ty: ValType::I64,
            mutable: false,
            value: Cell::new(WasmValue::from_u64(666)),
        })),
    );
    exports.insert(
        "global_f32".into(),
        ExportValue::Global(Rc::new(WasmGlobal {
            ty: ValType::F32,
            mutable: false,
            value: Cell::new(WasmValue::from_f32(666.6)),
        })),
    );
    exports.insert(
        "global_f64".into(),
        ExportValue::Global(Rc::new(WasmGlobal {
            ty: ValType::F64,
            mutable: false,
            value: Cell::new(WasmValue::from_f64(666.6)),
        })),
    );

    exports
        .insert("table".into(), ExportValue::Table(Rc::new(RefCell::new(WasmTable::new(10, 20)))));
    exports
        .insert("memory".into(), ExportValue::Memory(Rc::new(RefCell::new(WasmMemory::new(1, 2)))));
    ...
    exports.insert(
        "print_i32".into(),
        make_fn(Signature { params: vec![ValType::I32], result: None }),
    );
    exports.insert(
        "print_i64".into(),
        make_fn(Signature { params: vec![ValType::I64], result: None }),
    );
    exports.insert(
        "print_f32".into(),
        make_fn(Signature { params: vec![ValType::F32], result: None }),
    );
    exports.insert(
        "print_f64".into(),
        make_fn(Signature { params: vec![ValType::F64], result: None }),
    );
    exports.insert(
        "print_i32_f32".into(),
        make_fn(Signature { params: vec![ValType::I32, ValType::F32], result: None }),
    );
    exports.insert(
        "print_f64_f64".into(),
        make_fn(Signature { params: vec![ValType::F64, ValType::F64], result: None }),
    );
...
fn exec_action(
    instances: &HashMap<String, Rc<Instance>>,
    action: &Act,
) -> Result<Vec<WasmValue>, Error> {
    let (module_name, field, args) = match action {
        Act::Get { module, field } => {
            (module.as_deref().unwrap_or("default"), field.as_str(), vec![])
        }
        Act::Invoke { module, field, args } => {
            (module.as_deref().unwrap_or("default"), field.as_str(), to_wasm_values(args))
        }
    };
...
fn check_results(results: &[WasmValue], expected: &[ValueJSON]) -> Result<(), String> {
    ...
    for (i, ((result, exp_val), exp_json)) in
        results.iter().zip(&exp_values).zip(expected).enumerate()
    {
    ...
    for cmd in &wast.commands {
        let result = match cmd {
            TestCmd::Module { name, filename, .. } => {
                ...
                let module =
                    Module::compile(bytes).map_err(|e| format!("compile failed: {}", e))?;
            ...
            TestCmd::Register { name, r#as, .. } => {
                ...
                let inst =
                    instances.get(key).ok_or_else(|| format!("module '{}' not found", key))?;
            ...
            TestCmd::AssertReturn { action, expected, .. } => exec_action(&instances, action)
            ...
            TestCmd::AssertExhaustion { action, .. } => match exec_action(&instances, action) {
...
fn run_spec_tests() {
    ...
    for entry in fs::read_dir(test_dir).expect("failed to read test directory") {
        ...
        match run_test_file(&json_path, stem) {
            Ok((passes, message_mismatches, failures)) => {
                ...
                println!(
                    "  {} passed, {} had error message mismatch, {} failed",
                    passes, message_mismatches, failures
                );
            }
            Err(e) => {
                eprintln!("  Error: {}", e);
    ...
    println!(
        "\nTotal: {} passed, {} had error message mismatch, {} failed",
        total_passes, total_mismatches, total_failures
    );

    if total_failures > 0 || total_mismatches > 0 {
        panic!(
            "{} tests failed and {} tests had error message mismatch",
            total_failures, total_mismatches
        );
    }
}
