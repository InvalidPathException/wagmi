...
fn read_leb128_u64(bytes: &[u8], mut pos: usize) -> Result<(u64, usize), Error> {
    ...
    let mut shift = 0;
    loop {
        let byte = *bytes.get(pos).ok_or_else(|| Error::Malformed("unexpected end of section or function".into()))?;
        pos += 1;
...
fn read_leb128_i64(bytes: &[u8], mut pos: usize) -> Result<(i64, usize), Error> {
    ...
    let mut byte;
    loop {
        byte = *bytes.get(pos).ok_or_else(|| Error::Malformed("unexpected end of section or function".into()))?;
        pos += 1;
        if shift < 63 {
...
where T: TryFrom<u64> {
    ...
    let consumed = end - *pc;
    if consumed > (bits as usize + 6) / 7 { return malformed("integer representation too long"); }

    // Only bits=1 and bits=32 are used
    if (bits == 1 && result > 1 ) || (bits == 32 && result > 0xFFFFFFFF) { return malformed("integer too large"); }

    if consumed > 1 {
        let used = (consumed - 1) * 7;
        if used < bits as usize {
            let rem = bits as usize - used;
            if rem < 32 && (bytes[end - 1] as u32) >> rem != 0 { return malformed("integer too large"); }
        }
    }
    *pc = end;
...
where T: TryFrom<i64> {
    ...
    match bits { // Only bits=32, 33, 64 are used
        32 | 33 => {
            ...
            if (result as i128) < MIN_I32 || (result as i128) > MAX_I32 { return malformed("integer too large"); }
        }
        64 => {} // Already i64
        _ => unreachable!()
    }
    ...
    if consumed >= 1 {
        ...
        if ((last != 0 && last != 127) as usize + (consumed - 1) * 7) >= bits as usize {
            return malformed("integer too large");
        }
    }
    *pc = end;
    Ok(T::try_from(result).ok().unwrap())
