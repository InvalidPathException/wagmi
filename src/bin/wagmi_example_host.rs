...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let print_fn = RuntimeFunction::new_host(vec![ValType::I32], None, move |args| {
    ...
    let state_clone = host_state.clone();
    let random_fn = RuntimeFunction::new_host(vec![], Some(ValType::I32), move |_args| {
        use std::time::{SystemTime, UNIX_EPOCH};
        let seed = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().subsec_nanos() as i32;
    ...
    let state_clone = host_state.clone();
    let counter_inc_fn = RuntimeFunction::new_host(vec![], Some(ValType::I32), move |_args| {
    ...
    let state_clone = host_state.clone();
    let counter_get_fn = RuntimeFunction::new_host(vec![], Some(ValType::I32), move |_args| {
