...
macro_rules! tail_call_dispatch {
    ($instance:ident, $pc:ident, $stack:ident, $control:ident, $func_bases:ident, $ctrl_bases:ident) => {
        become dispatch($instance, $pc, $stack, $control, $func_bases, $ctrl_bases)
    };
}
...
macro_rules! binary_handler {
    ($name:ident, $ty:ident, method $method:ident) => {
        handler_fn!($name, |instance, pc, stack, control, func_bases, ctrl_bases| {
            ...
            tail_call_dispatch!(instance, pc, stack, control, func_bases, ctrl_bases)
        });
    };
...
handler_fn!(op_block, |instance, pc, stack, control, func_bases, ctrl_bases| {
    ...
    tail_call_dispatch!(instance, body_pc, stack, control, func_bases, ctrl_bases)
});
...
handler_fn!(op_if, |instance, pc, stack, control, func_bases, ctrl_bases| {
    ...
    tail_call_dispatch!(instance, next, stack, control, func_bases, ctrl_bases)
});

handler_fn!(op_else, |instance, pc, stack, control, func_bases, ctrl_bases| {
    let _ = Instance::branch(&mut (pc.clone()), stack, control, 0);
    tail_call_dispatch!(instance, pc, stack, control, func_bases, ctrl_bases)
});
...
handler_fn!(op_end, |instance, pc, stack, control, func_bases, ctrl_bases| {
    if let Some(&frame_idx) = ctrl_bases.last() {
        if frame_idx == control.len().saturating_sub(1) {
            if Instance::branch(&mut (pc.clone()), stack, control, 0) {
                ctrl_bases.pop();
...
handler_fn!(op_return, |instance, pc, stack, control, func_bases, ctrl_bases| {
    ...
    let mut pc2 = pc;
    if Instance::branch(&mut pc2, stack, control, depth) {
        ctrl_bases.pop();
        let _ = func_bases.pop();
        return Ok(());
    }
...
handler_fn!(op_call, |instance, pc, stack, control, func_bases, ctrl_bases| {
    ...
    match func {
        RuntimeFunction::OwnedWasm { runtime_sig, pc_start, locals_count } => {
            ...
            tail_call_dispatch!(instance, new_pc, stack, control, func_bases, ctrl_bases)
        }
...
fn dispatch(
    ...
    let bytes = &instance.module.bytes;
    let opcode = unsafe { *bytes.get_unchecked(pc) };
    pc += 1;
    let handler = HANDLERS[opcode as usize];
    become handler(instance, pc, stack, control, func_bases, ctrl_bases);
}
...
impl Instance {
    ...
    fn interpret(
        ...
        dispatch(self, pc, stack, control, func_bases, ctrl_bases)
    }
